...
const MODE_CONFIG: Record<ChatMode, { label: string; icon: any; desc: string; gradient: string }> = {
  'pre-sales': {
    label: '售前咨询',
    icon: ShoppingCart,
    desc: '产品选型、价格咨询、货期查询',
...
export default function ChatPage() {
  ...
  const [mode, setMode] = useState<ChatMode>(urlMode && MODE_CONFIG[urlMode] ? urlMode : 'pre-sales')
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 'welcome',
      role: 'assistant',
      content: '您好！我是 AIMENG UNING 爱萌优宁智能客服助手。请选择上方的服务模式，我将为您提供专业的 ELISA 相关咨询服务。',
    },
  ])
  ...
  useEffect(() => {
    if (urlMode && MODE_CONFIG[urlMode] && urlMode !== mode) {
      setMode(urlMode)
      setMessages([
        {
          id: 'welcome',
  ...
  return (
    <div className="min-h-screen bg-[#F2F6FA] text-[#1E293B] flex flex-col">
      <Navbar />
      
      <div className="pt-16 bg-white border-b border-gray-200 px-4 py-4">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h1 className="text-lg font-semibold text-[#1E293B]">RAG AI 智能客服</h1>
              <p className="text-xs text-[#94A3B8]">DeepSeek 大模型驱动 · 专业 ELISA 咨询服务</p>
            </div>
          </div>
            ...
            <div key={msg.id} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
              <div
                className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                  msg.role === 'user'
                    ? 'bg-blue-600'
                    : 'bg-gradient-to-br from-[#3CB5C0] to-[#2563EB]'
                }`}
              >
                {msg.role === 'user' ? (
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-lg transition-colors ${
                        msg.rating === 'up'
                          ...
                          : 'text-[#94A3B8] hover:text-[#475569] hover:bg-gray-100'
                      }`}
                    >
