  ...
  return (
    ...
    <div className="max-w-4xl mx-auto px-4 py-10">
      <p className="font-mono text-xs font-semibold uppercase tracking-[0.28em] text-teal-700">
        aimeng.member / my account
      </p>
      <h1 className="mt-2 text-2xl font-black tracking-normal text-slate-950 mb-6">会员中心</h1>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-lg p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-semibold text-slate-900">{currentTier.label}</div>
                  <div className="text-xs text-slate-500">{currentTier.discount}</div>
                </div>
              </div>
              <Link
                href="/leaderboard"
            ...
            <div className="mt-4 rounded-lg border border-teal-100 bg-teal-50 px-3 py-3 text-xs leading-5 text-teal-900">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {TIERS.map((tierItem) => {
              ...
              return (
                <div
                  key={tierItem.name}
                  className={`rounded-xl border p-3 ${active ? 'border-teal-200 bg-teal-50' : 'border-slate-200 bg-white'}`}
                >
                  <div className="mb-2">
                    <MemberTierIcon src={tierItem.image} label={tierItem.label} size="sm" />
                  </div>
                  <div className="text-sm font-semibold text-slate-900">{tierItem.label}</div>
                  <div className="mt-1 text-xs text-slate-500">累计 {tierItem.min}+ 积分</div>
                  <div className="mt-1 text-xs font-medium text-teal-700">{tierItem.discount}</div>
                </div>
              )
            })}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Link
              ...
              className="flex items-center gap-3 p-4 bg-white border border-slate-200 rounded-lg hover:border-teal-300 transition-colors"
            >
              <Ticket className="w-5 h-5 text-teal-700" />
              <div>
                <div className="text-sm font-medium text-slate-900">购买积分</div>
                <div className="text-xs text-slate-500">积分码 + 商品照片审核</div>
              </div>
            </Link>
            <Link
            ...
            >
              ...
              <div>
                <div className="text-sm font-medium text-slate-900">提交文献</div>
                <div className="text-xs text-slate-500">审核通过最高 +1500 积分</div>
              </div>
            </Link>
            <Link
              href="/store"
            ...
            >
              ...
              <div>
                <div className="text-sm font-medium text-slate-900">积分商城</div>
                <div className="text-xs text-slate-500">兑换实验耗材</div>
              </div>
            </Link>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-200 flex items-center justify-between">
              <h2 className="text-sm font-semibold text-slate-900">我的文献</h2>
              <Link href="/user/citations" className="text-xs text-teal-700 hover:text-teal-800">
                查看全部
              </Link>
            </div>
            <div className="p-4">
              ...
              <Link
                href="/user/citations/submit"
                className="flex items-center justify-center gap-2 w-full py-2.5 bg-slate-950 text-white rounded-lg text-sm font-bold hover:bg-teal-700 transition-colors"
              >
          ...
          <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-200">
              <h2 className="text-sm font-semibold text-slate-900">兑换记录</h2>
            </div>
            {orders.length === 0 ? (
              <div className="px-4 py-6 text-center text-sm text-slate-400">暂无兑换记录</div>
            ) : (
              <div className="divide-y divide-gray-100">
                {orders.slice(0, 5).map((o) => (
                  <div key={o.id} className="px-4 py-3 flex items-center justify-between">
                    <div className="min-w-0">
                      <div className="text-sm text-slate-900">{o.item_name}</div>
